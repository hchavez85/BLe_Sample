...
//		}
		...
		if (enable) {
			Handler startt = new Handler();
			startt.postDelayed(new Runnable() {
				@Override
				public void run() {
//					if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP)
//                		mBluetoothAdapter.getBluetoothLeScanner().stopScan(scanCallback);
//            		else
	...
	private ScanCallback scanCallback = new ScanCallback() {
		...
		public void onScanResult(int callbackType, final ScanResult result) {
			...
			final EddystoneClass.Eddystone device = EddystoneClass.fromScanData(result.getDevice(), result.getRssi(), result.getScanRecord().getBytes());
			runOnUiThread(new Runnable() {
				@Override
				public void run() {
					if (device!=null && device.type.equalsIgnoreCase("00112233445566778899")){
						System.out.println(device.rssi+"\t time:"+result.getTimestampNanos());
					}
